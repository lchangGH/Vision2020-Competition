...
folder = "/Users/rachellucyshyn/Documents/GitHub/Vision2020-Competition/PowerCellImages"

# read and filter file names
filenames1 = glob.glob(folder + '/' + '*.[jJ][pP][gG]')
filenames2 = glob.glob(folder + '/' + '*.[jJ][pP][eE][gG]')
filenames3 = glob.glob(folder + '/' + '*.[pP][nN][gG]')
...
while(True):
    ...
    imgBGRInput = cv2.imread(filenames[i])
    ...
    if k == 97: # user wants to move down list (a)
        i = (i - 1) % len(imgShowMaths) # mod (%) gives the remainder-if you go back down it'll give you the last thing
    if k == 122: # user wants to move up list (z)
        i = (i + 1) % len(imgShowMaths)
